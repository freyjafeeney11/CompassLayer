...
# Import NavIcon from the existing architecture to maintain compatibility
from core.audiofeedback import NavIcon

class OfflineAudioRenderer:
    """
    Renders audio events to a single stereo wav buffer for offline video export.
    Uses equal-power panning to match the spatial audio feedback in real-time mode.
    """
    SAMPLE_RATE = 44100
    
    def __init__(self, total_duration_sec: float):
        self.total_samples = int(total_duration_sec * self.SAMPLE_RATE)
        # Initialize an empty stereo buffer
        ...
        self.sounds = {
            "quest": self._load_sound(os.path.join(audio_dir, "koto_note.wav"), 0.4),
            "quest_center": self._load_sound(self._get_or_create_center_sound(audio_dir), 0.4),
            "treasure": self._load_sound(os.path.join(audio_dir, "koto_trill.wav"), 0.4),
            "stockpile": self._load_sound(os.path.join(audio_dir, "koto_stockpile.wav"), 0.2)
        }
...
class OfflineNavigationController:
    """
    Mimics NavigationController but evaluates cooldowns and pulse intervals 
    deterministically using video timestamps rather than time.time().
    """
    ...
    def update(self, icons: List[NavIcon], current_time: float):
        ...
        if quest:
            ...
            # Pulse continuously based on calculated interval
